...
У каждого Task должен быть непустой executors. anonymous_named используй только для явно названного в брифе человека, команды или компании. Если указана только профессия или роль, используй anonymous_unnamed с role и без name; никогда не копируй role или профессию в name. Только явное «из базы»/«из библиотеки» означает performer_binding. Не помещай имя, роль, оплату или налог исполнителя в Task.name. Несколько исполнителей — несколько drafts; допустим count/copies 1..10, которые будут нормализованы. Не возвращай performerId, snapshots, tags, trusted IDs или low-level operations. Не возвращай task.cost: компенсация относится к конкретному ExecutorDraft.
`;
