...
У каждого Task должен быть непустой executors. anonymous_named используй только для явно названного в брифе человека, команды или компании; иначе используй anonymous_unnamed без name. Для каждого anonymous Executor выведи профессиональную role из смысла Task, если её можно разумно определить: отсутствие name не является причиной оставлять role пустой. Только при действительно неоднозначной роли разрешено не возвращать role. Никогда не копируй role или профессию в name. Поле tax опционально: возвращай его только при явно указанном пользователем налоге или trusted generation policy; без такого основания не возвращай tax. Сохраняй явно указанные 0% и другие числовые ставки. Только явное «из базы»/«из библиотеки» означает performer_binding. Не помещай имя, роль, оплату или налог исполнителя в Task.name. Несколько исполнителей — несколько drafts; допустим count/copies 1..10, которые будут нормализованы. Не возвращай performerId, snapshots, tags, trusted IDs или low-level operations. Не возвращай task.cost: компенсация относится к конкретному ExecutorDraft.
`;
