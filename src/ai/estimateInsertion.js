...
import { generatedPaymentSemantics } from "./generatedPayment.js";
...
function materializeDraft(draft, performers, policy) {
  if (draft.type === "performer_binding") {
    ...
    const executor = buildExecutorFromPerformer(matches[0], { inheritFinancials: policy.performerRateMode !== "leave_blank" }); executor.id = uid(); executor.tags = executor.tags.map((tag) => ({ ...tag, id: uid() })); return executor;
  }
  const { type, quantityField } = generatedPaymentSemantics(draft), tags = [];
  ...
  if (type) {
    ...
    if (draft.quantity !== undefined && quantityField) tag.payment[quantityField] = String(draft.quantity);
    tags.push(tag);
  }
  return { id: uid(), amount: type === "fix_total" && draft.compensation !== undefined ? String(Math.round(Number(draft.compensation))) : "", tags };
}

export function stagesFromGeneratedEstimate(parsed, performers = [], policy = {}) {
  return parsed.stages.map((stage) => ({
    id: uid(), presetKey: "custom", collapsed: false, name: stage.name || "Смета",
    tasks: stage.tasks.map((task) => ({ id: uid(), name: task.name || "", markupOverride: null, executors: (task.executors || [{ type: "anonymous_unnamed", paymentType: "fix_total", compensation: task.cost }]).map((draft) => materializeDraft(draft, performers, policy)) })),
  }));
}
