...
export function AiEditTechnicalModal({ scope, onRequest, onCancelRequest, onApply, onClose }) {
  ...
  const request = async () => {
    ...
    try { setResult(await onRequest({ scope, instruction })); setState("ready"); }
  ...
  return <div className="kb-modal-overlay" onMouseDown={busy ? undefined : onClose}>
    <div className="kb-modal kb-ai-settings-modal" role="dialog" aria-modal="true" aria-labelledby="ai-edit-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="kb-modal-body">
        ...
        {result?.kind === "clarification" && <div className="kb-modal-note"><strong>Нужно уточнение:</strong> {result.question}</div>}
        ...
        <div className="kb-modal-actions">
          ...
          {result && result.kind !== "diff" && <button type="button" className="kb-btn kb-btn-primary" onClick={() => { setResult(null); setError(""); }}>Новый запрос</button>}
        </div>
      </div>
    </div>
  </div>;
