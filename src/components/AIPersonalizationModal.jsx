...
export function AIPersonalizationModal({ settings, state = "ready", message = "", onSave, onClose }) {
  ...
  useEffect(() => {
    if (!dirtyRef.current) setDraft(normalizeAiSettings(settings));
  }, [settings]);
  const saving = state === "saving";
  const save = async () => {
    if (await onSave(draft)) dirtyRef.current = false;
  };
