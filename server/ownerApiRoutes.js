...
export async function handleOwnerApiRoute(route, request, repository, userId) {
  ...
  if(name==="PUT /api/export-profile"){const raw=payload(request).profile??payload(request), s=normalizePresentationSettings({branding:raw});const profile={company_name:s.branding.companyName,logo_asset_path:s.branding.logoAssetPath||null,logo_position:s.branding.logoPosition,phone:s.branding.phone,email:s.branding.email,website:s.branding.website,default_colors:s.branding.colors,default_font:s.branding.fontFamily,default_legal_text:boundedObject(raw.defaultLegalText??{},16000)};return response(200,await repository.upsertExportProfile(userId,profile));}
